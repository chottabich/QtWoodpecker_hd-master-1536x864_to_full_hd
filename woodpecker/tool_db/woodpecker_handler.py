...
from tool_db import Tool_Database as TOOL_TABLE
...
class HandlerClass:
    def __init__(self, halcomp, widgets, paths):
        ...
        self.tab_index_code = (0, 1, 2, 3, 0, 5, 2, 0, 1, 0, 4, 5, 0)
...
""" %(os.path.join(paths.IMAGEDIR,'lcnc_swoop.png'))
    ...
    def initialized__(self):
        ...
        self.w.btn_enable_edit.clicked.connect(lambda state: self.enable_edit_clicked(state))
    ...
    def init_utils(self):
        ...
        from tool_db import Tool_Database
        self.tool_db = Tool_Database()
        version = self.tool_db.get_version()
        self.w.layout_tool_offset.insertWidget(0, self.tool_db)
    ...
    def init_widgets(self):
        ...
        if os.path.isdir(path):
            ...
            LOG.info("No tool icons found in {}".format(path))
...
#                    QtCore.Qt.Key_F3,QtCore.Qt.Key_F4,QtCore.Qt.Key_F5):
    ...
    def tool_changed(self, tool):
        ...
        self.w.lineEdit_max_depth.setText(maxz)
    ...
    def pause_spindle(self):
        ...
        self.add_status("Spindle resumed")
    ...
    def btn_add_tool_pressed(self):
        if not STATUS.is_on_and_idle():
            LOG.debug("Status must be ON and IDLE")
            return
    ...
    def btn_delete_tool_pressed(self):
        if not STATUS.is_on_and_idle():
            LOG.debug("Status must be ON and IDLE")
            return
        tools = self.tool_db.get_checked_tools()
        if len(tools) == 0:
            self.add_status("No tools selected to delete")
            return
        for tool in tools:
            self.tool_db.delete_tool(tool)
        self.get_next_available()

    def btn_load_tool_pressed(self):
        tool = self.tool_db.get_checked_tools()
        if len(tool) == 0:
            self.add_status("No tool selected")
        elif len(tool) > 1:
            self.add_status("Select only 1 tool")
        else:
            ACTION.CALL_MDI(f"M61 Q{tool} G43")

    def btn_unload_tool_pressed(self):
        ACTION.CALL_MDI("M61 Q0")

    def enable_edit_clicked(self, state):
        self.tool_db.set_edit_mode(state)

    def export_data_pressed(self):
        self.tool_db.export_data()

    def icon_select_activated(self, index):
        if index == 0: return
        icon = self.w.cmb_icon_select.currentText()
        self.tool_db.set_tool_icon(icon)
...
#        if self.w.isFullScreen() == False:
    ...
    def get_next_available(self):
        #self.tool_list = self.tool_db.get_tool_list()
        #for tno in range(0, 100):
            #if tno not in self.tool_list: break
        self.next_available = self.tool_db.get_next_available()
        self.w.lineEdit_next_available.setText(str(self.next_available))
